...
screened_hparams = ["uid", "log_dir", "exp_name", "run", "dataset", "data_dir", "model_name", "model_path", "seed",
                    "train_batch_size", "dropout", "encoder_hidden_size", "arg_comp_hidden_size", "num_classes", "loss",
                    "aug", "aug_prob",
...
dynamic_hparams = ["log_dir", "model_path", "cur_epoch", "train_loss", "train_acc", "train_auc", "val_loss", "val_acc", "val_auc", "strategy",
                   "gpus"]
...
class CSVModelCheckpoint(ModelCheckpoint):
    ...
    def _update_best_and_save(self, current, trainer, monitor_candidates):
        ...
        df.loc[df["uid"] == self.cur_uid, "train_acc"] = monitor_candidates["train_acc_epoch"].detach().cpu().numpy()
        df.loc[df["uid"] == self.cur_uid, "train_auc"] = monitor_candidates["train_auc_epoch"].detach().cpu().numpy()
        df.loc[df["uid"] == self.cur_uid, "val_loss"] = monitor_candidates["val_epoch_loss"].detach().cpu().numpy()
        df.loc[df["uid"] == self.cur_uid, "val_acc"] = monitor_candidates["val_epoch_acc"].detach().cpu().numpy()
        df.loc[df["uid"] == self.cur_uid, "val_auc"] = monitor_candidates["val_epoch_auc"].detach().cpu().numpy()
