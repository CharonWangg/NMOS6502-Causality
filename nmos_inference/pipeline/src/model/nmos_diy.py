...
# TCN Network
class NmosDiy(pl.LightningModule):
    ...
    def __init__(self, input_size=None,
        ...
        self.encoder = TemporalConvNet(self.input_size, self.encoder_hidden_size)
        self.dropout = nn.Dropout(p=0.2)
        self.att = nn.MultiheadAttention(self.input_size, num_heads=1, batch_first=True)
    ...
    def forward(self, seq):
        ...
        seq = self.att(seq, seq, seq)[0].permute(0, 2, 1)
        seq = self.encoder(seq).permute(0, 2, 1)
        seq = self.linear(seq)
        return seq
